...
<div class="page-content">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-control @error('user_id') is-invalid @enderror" id="user_id" name="user_id" required>
                                        ...
                                        @foreach($users as $user)
                                            <option value="{{ $user->id }}" {{ old('user_id', $incident->user_id) == $user->id ? 'selected' : '' }}>
                                                {{ $user->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control @error('deceased_name') is-invalid @enderror"
                                        ...
                                        value="{{ old('deceased_name', $incident->deceased_name) }}" required>
                                    @error('deceased_name')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="date" class="form-control @error('date_of_death') is-invalid @enderror"
                                        ...
                                        value="{{ old('date_of_death', $incident->date_of_death) }}" required>
                                    @error('date_of_death')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
    ...
    <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" required>
        ...
        @foreach($types as $type)
            <option value="{{ $type }}" {{ old('type', $incident->type) == $type ? 'selected' : '' }}>
                {{ ucfirst($type) }}
            </option>
        @endforeach
    </select>
    ...
    @enderror
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" step="0.01" class="form-control @error('amount') is-invalid @enderror"
                                        id="amount" name="amount" value="{{ old('amount', $incident->amount) }}" required>
                                    @error('amount')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror"
                                        id="description" name="description" rows="3" required>{{ old('description', $incident->description) }}</textarea>
                                    @error('description')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-control @error('status_id') is-invalid @enderror"
                                        ...
                                        @foreach($statuses as $status)
                                            <option value="{{ $status->id }}" {{ old('status_id', $incident->status_id) == $status->id ? 'selected' : '' }}>
                                                {{ $status->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-control @error('verified_by') is-invalid @enderror"
                                        ...
                                        @foreach($users as $user)
                                            <option value="{{ $user->id }}" {{ old('verified_by', $incident->verified_by) == $user->id ? 'selected' : '' }}>
                                                {{ $user->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-12 mb-3">
                                    <div class="alert alert-info">
                                        <strong>Current Status:</strong> {{ $incident->status->name ?? 'N/A' }}
                                        <br><small class="text-muted">Only administrators can change the status and verification details.</small>
                                    </div>
                                </div>
                                @endif
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    @endif
                                    <label for="thumbnail_path" class="form-label">Update Thumbnail</label>
                                    ...
                                    <div class="row">
                                        @foreach($incident->images as $image)
                                            <div class="col-md-3 mb-3">
                                                <div class="card">
                                                    <img src="{{ Storage::url($image->image_path) }}" class="card-img-top" alt="Event Image">
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                ...
                                @if(hasPermissionOrRole('update-incident'))
                                    <button type="submit" class="btn btn-primary">
                                        <span class="fe fe-save"></span> Update Event
                                    </button>
                                @endif
                            </div>
    ...
    $(document).ready(function() {
        $('.delete-image').click(function() {
            if (confirm('Are you sure you want to delete this image?')) {
                ...
                // Add your AJAX deletion logic here
            }
        });
    });
</script>
