                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.blog-tags.update', $blogTag) }}" method="POST">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-control @error('status') is-invalid @enderror" id="status" name="status">
                                        <option value="1" {{ old('status', $blogTag->status) == '1' ? 'selected' : '' }}>Active</option>
                                        <option value="0" {{ old('status', $blogTag->status) == '0' ? 'selected' : '' }}>Inactive</option>
                                    </select>
                                    @error('status')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                <a href="{{ route('admin.blog-tags.index') }}" class="btn btn-light">Cancel</a>
